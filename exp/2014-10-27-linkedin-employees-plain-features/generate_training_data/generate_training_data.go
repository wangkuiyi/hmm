...
func GenerateJSON(exps map[string][]*Record) []*core.Instance {
	...
	for memberSk, exp := range exps {
		...
		inst := &core.Instance{
			Obs:     makeObservedMatrix(years, kChannels),
			Periods: ones(years)}
...
func feature(r *Record, ch int) string {
	base := ""
	switch ch {
	case 0:
